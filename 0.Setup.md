...
Makale hazırlanırken 


### LXC İle Ortam Kurulumu

Dikkat edilirse bridge network'ler default olarak her network'ün sonu 1 olan ip'sini kendisi alıyor. firewall üzerinde her network'ten gelen bağlantılar için sonu 10 olan ip'leri kullanıyoruz bunlar aynı zamanda gateway oluyor makinalar için.

![NetworkIptables.jpg](files/NetworkIptables.jpg)

Makinalar üzerinde tanımlı olacak network'ler 
...
Dört adet makinanın indirildiğini ve stop haliyle LXC'de olduğunu ve default profile'ları da kullanmadığını varsayıyoruz. Yada kullanıyorsa bile network ayarlarının boş bırakıyoruz.
...
# internet lazım olabilir diye firewall makinası için bir nat network oluşturuyoruz. Gerek olmayan durumlarda down edilebilir
...
Daha sonra bu network'leri listeki makinalara uygun şekilde attach edip iplerini atıyoruz.
...
Sistem tarafından eth0'lara atanan otomatik ip yi statiğe çekmek için alttaki komutları kullanıyoruz. Ayrıca firewall üzerinde eth1'leri de statik hale getirdiğimizden emin oluyoruz. Ancak yine de firewall üzerinde makina içinde _ip a add ip_address/24 dev eth1_ komutu ile ip ataması da yapmak gerekecek. Bunu daha kalıcı yapmanın en iyi yolu normalde Canonical firmasının geliştirdiği [cloud-init](https://cloud-init.io/) aracı ancak şuan konumuz değil. Detaylara sayfasından bakabilirsiniz.
...
```
Son olarak emin olmak için  makinalardaki default gateway'lerin şemadaki ile aynı olduğundan emin olmalıyız. Persistent (kalıcı) olarak yapma yolu da aşağıda var
...
Kurduğumuz network yapısında makinaların ortak kullandıkları network'ler için bridge kullanılır. Bridge network'ü makinalara bağlamak için genel kabül tun/tap kullanmaktır. Böylece makinalar doğrudan switch'e bağlıymış gibi olur. 

Alttaki şekilde görüleceği üzere bridge tarafından bir kablo çıkartılıp makinalara takılmış gibi bir durum var. Burada br0 üzerinde bizzat br0 a ait bir ip aslında yok. Vm1 üzerindeki eth0'ın ip'si tune/tap ip'si olmuş oluyor.
...
Makinaları ayağa kaldırdıktan sonra sunucularda bazı servislerin çalışması gerekiyor.
